...
export const BoostModal = ({ isVisible, onClose, item, onSuccess }: BoostModalProps) => {
    ...
    const handleBoost = async () => {
        if (!user) {
            Alert.alert("Error", "You must be logged in to boost.");
            return;
        }
        setLoading(true);
        ...
        try {
            ...
            Alert.alert("Success", `Your item has been boosted for ${selectedTier.label}!`);
            onSuccess();
            onClose();
                ...
                <View className="bg-white dark:bg-gray-900 rounded-t-3xl p-6 h-[55%]">
                    ...
                    <View className="flex-row justify-between items-center mb-6">
                        <View className="flex-row items-center">
                            ...
                            <Text className="text-gray-900 dark:text-white text-xl font-bold">Boost Listing</Text>
                        </View>
                    ...
                    <Text className="text-gray-500 dark:text-gray-400 mb-6">
                        Get more views by pinning "{item.title}" to the top of the feed.
                    </Text>
                    ...
                    <View className="flex-row flex-wrap justify-between gap-3 mb-8">
                        {BOOST_TIERS.map((tier) => (
                            <TouchableOpacity
                                ...
                                className={`w-[48%] p-4 rounded-xl border-2 ${selectedTier.duration === tier.duration
                                ...
                                {tier.recommended && (
                                    <View className="absolute -top-3 left-0 right-0 items-center">
                                        <View className="bg-yellow-500 px-2 py-0.5 rounded-full">
                                            <Text className="text-white text-[10px] font-bold uppercase">Best Value</Text>
                                        </View>
                                    </View>
                                )}
                                <Text className={`font-bold text-lg mb-1 ${selectedTier.duration === tier.duration
