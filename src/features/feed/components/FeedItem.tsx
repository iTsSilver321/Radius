...
export const FeedItem = React.memo(
  ({ item, onPress }: FeedItemProps) => {
    const userLocation = useLocationStore((state) => state.location);
    // console.log("Rendering FeedItem", item.id); // Uncomment for debugging
  ...
  (prevProps, nextProps) => {
    ...
    // Only re-render if item props changed deeply or onPress changed
    return (
