...
import { ArrowLeft, Flag, MapPin, MessageCircle, Settings, Share2, ShieldCheck, Star, Zap } from "lucide-react-native"; // Added Flag, Zap
...
export default function ItemDetailsScreen() {
    ...
    const handleMessageSeller = async () => {
        ...
        if (user.id === item.owner_id) {
            router.push({ pathname: "/item/edit/[id]", params: { id: item.id } } as any);
            return;
        }
        ...
        try {
            ...
            Alert.alert("Error", "Could not contact seller. Please try again.");
        } finally {
            setContacting(false);
        }
    };
            ...
            <ScrollView className="flex-1 -mt-6 rounded-t-3xl bg-gray-50 px-6 pt-8 dark:bg-background">
                ...
                <View className="flex-row items-center justify-between mb-8 pb-6 border-b border-gray-200 dark:border-white/10">
                    <TouchableOpacity
                        ...
                        onPress={() => router.push({ pathname: "/user/[id]", params: { id: item.owner_id } })}
                    >
                        <Image
                            source={{
                                uri: item.owner?.avatar_url || `https://ui-avatars.com/api/?name=${item.owner?.full_name || "User"}&background=random`,
                            }}
                ...
                <View className="bg-blue-50 p-4 rounded-2xl mb-24 dark:bg-blue-900/20">
                    <Text className="text-blue-800 font-bold mb-1 dark:text-blue-300">Safety Tip</Text>
                    <Text className="text-blue-600 text-sm dark:text-blue-200">
                        Always meet in a public place. Do not transfer money before seeing the item.
                    </Text>
                </View>
            </ScrollView>
            ...
            <View className="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-100 dark:bg-card dark:border-white/5">
                {user?.id === item.owner_id ? (
                    <View className="gap-3">
                        <View className="flex-row gap-3">
                            <Button
                                title={item.status === 'active' ? "Mark Reserved" : "Mark Active"}
                                onPress={async () => {
                                    if (!item) return;
                                    try {
                                        const newStatus = item.status === 'active' ? 'reserved' : 'active';
                                        const { updateItemStatus } = await import("@/src/features/feed/api");
                        ...
                        {item.status !== 'sold' && (
                            <Button
                                ...
                                onPress={() => {
                                    if (item.is_boosted) {
                                        Alert.alert("Info", "This item is already boosted.");
                                        return;
                                    }
                                    setIsBoostModalVisible(true);
                                }}
                                variant={item.is_boosted ? "outline" : "primary"}
                                icon={<Zap size={20} color={item.is_boosted ? "#eab308" : "white"} />}
                                className={item.is_boosted ? "border-yellow-500" : "bg-yellow-500"}
                        ...
                        {item.status === 'sold' ? (
                            <View className="gap-3">
                                <View className="bg-gray-100 dark:bg-white/10 p-4 rounded-xl items-center">
                                    <Text className="text-gray-500 font-bold text-lg">Item Sold</Text>
                                </View>
                                <Button
                                    title="Rate Seller"
